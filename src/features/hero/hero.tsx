...
export function Hero() {
  return (
    <section className="relative overflow-hidden">
      ...
      <div
        aria-hidden
        className="pointer-events-none absolute -top-40 left-1/2 h-[520px] w-[820px] -translate-x-1/2 rounded-full bg-brand-600/20 blur-[130px]"
      />
      ...
      <Container className="relative grid items-center gap-16 py-20 lg:grid-cols-2 lg:py-28">
        <div className="animate-fade-up">
          ...
          <h1 className="mt-6 text-4xl font-bold leading-[1.1] tracking-tight sm:text-5xl lg:text-[3.4rem]">
            ...
            <span className="text-gradient">{heroContent.titleAccent}</span>{' '}
          ...
          <div className="mt-10 flex flex-wrap items-center gap-4">
            <div className="flex -space-x-2">
              {heroContent.avatars.map((avatar) => (
                <Avatar
                  ...
                  className="h-8 w-8 border-2 border-ink-900 text-[10px]"
                />
              ))}
            </div>
            ...
            <div>
              <Stars />
              <p className="mt-1 text-xs text-fg-muted">{heroContent.socialProof}</p>
            </div>
          </div>
        </div>
